...
if (!(window as any).__opendevsVisuals) {
  ...
  function showFrame(rect?: { x: number; y: number; width: number; height: number } | null): void {
    ...
    frameEl.style.boxShadow =
      "inset 0 0 30px rgba(58, 150, 221, 0.06), 0 0 15px rgba(58, 150, 221, 0.08)";
    frameEl.style.transition = "opacity 150ms ease-out";
  ...
  function scanPage(): void {
    try {
      ...
      band1.style.cssText =
        ...
        BLUE +
        ",0.40) 100%);" +
        "box-shadow:0 4px 16px 2px rgba(" +
        BLUE +
        ",0.40)," +
        "0 2px 40px 6px rgba(" +
        BLUE +
      ...
      setTimeout(() => {
        try {
          ...
          band2.style.cssText =
            ...
            BLUE +
            ",0.50) 100%);" +
            "box-shadow:0 3px 14px 2px rgba(" +
            BLUE +
            ",0.50)," +
            "0 2px 36px 5px rgba(" +
            BLUE +
  ...
  function showActiveGlow(): void {
    try {
      ...
      if (!activeGlowEl || !activeGlowEl.parentNode) {
        ...
        activeGlowEl.style.boxShadow =
          "inset 0 0 80px 20px rgba(58, 150, 221, 0.35), inset 0 0 25px 8px rgba(58, 150, 221, 0.40)";
