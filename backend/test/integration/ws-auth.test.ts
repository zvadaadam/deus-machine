...
const { testDb, TEST_DB_PATH, TEST_DIR } = vi.hoisted(() => {
  ...
  const testDir = path.join(os.tmpdir(), `hive-test-ws-auth-${process.pid}-${Date.now()}`);
...
import { _clearAll as clearAuthState, createDeviceToken } from "../../src/services/auth.service";
