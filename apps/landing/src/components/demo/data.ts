  ...
  },
  {
    ...
    diff: {
      ...
      lines: [
        ...
        { type: 'add', content: '  const cacheKey = `users:${c.req.query("page")}:${c.req.query("q")}`' },
        ...
        { type: 'add', content: '  const users = await db.query("SELECT * FROM users LIMIT ? OFFSET ?", [limit, offset])' },
