...
description: Generate a hive.json manifest for a project. Auto-detects tech stack, package manager, scripts, and maps them to tasks with icons. Can import from existing OpenDevs or Codex configs. Use when setting up a new workspace or migrating from another tool.
...
Check for existing OpenDevs or Codex configurations and use them as the primary source. Imported values take precedence over auto-detected values.
