...
- `scripts/runtime/smoke-source-runtime.cjs`,
  `scripts/runtime/smoke-native-runtime.cjs`,
  `scripts/runtime/smoke-packaged-runtime.cjs`,
  `scripts/runtime/smoke-packaged-app.cjs`,
  `scripts/runtime/smoke-packaged-desktop.cjs`, and helpers under
  `scripts/runtime/lib/` are verification harnesses. They should share
  assertion constants when that removes duplication, but they are not production
  ownership layers.
...
- Keep shared runtime contracts in `shared/runtime.ts` or `shared/lib/cli-path.ts`
  ...
  agent-server.
- Keep smoke-only helpers in `scripts/runtime/lib/`. Do not move them into
...
- Keep public package script names stable. If a script moves, leave a wrapper at
...
- `scripts/runtime/` remains a flat command directory with reusable helpers under
  `scripts/runtime/lib/`. The command names map directly to package scripts, and
  moving them now would require compatibility wrappers without improving the
  release path.
