...
- `sidecar/db/schema.ts` — sidecar's schema mirror
- `sidecar/db/index.ts` — sidecar database access
- `src-tauri/src/main.rs` — app initialization and lifecycle
- `src-tauri/src/lib.rs` — module exports
- `src-tauri/src/socket*.rs` — sidecar socket relay
- `src-tauri/src/process*.rs` — process lifecycle management
- `sidecar/index.ts` — sidecar entry point
- `sidecar/rpc-connection.ts` — JSON-RPC communication
- `sidecar/protocol.ts` — shared message types

Required checks:
- `bun run typecheck`
- `bun run test` (all backend + sidecar tests)
- `cargo test --manifest-path src-tauri/Cargo.toml --lib`
- `bun run build:sidecar` (verify sidecar builds)
...
- `sidecar/agents/**` — agent handlers (Claude, Codex)
- `sidecar/frontend-client.ts` — frontend notifications
...
5. **Flag cross-boundary changes**: Warn if changes span both Rust and Node.js, or both backend and sidecar — these need extra attention
...
- Schema changes require both backend and sidecar schema sync
```
