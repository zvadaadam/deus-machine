...
describe('getAllSettings', () => {
  ...
  it('returns parsed settings when file exists', () => {
    ...
    mockFs.readFileSync.mockReturnValue(JSON.stringify({ theme: 'dark', onboarding_completed: true }));
    const result = getAllSettings();
  ...
  it('returns empty object when file contains invalid JSON', () => {
    mockFs.existsSync.mockReturnValue(true);
    mockFs.readFileSync.mockImplementation(() => { throw new Error('bad JSON'); });
    const result = getAllSettings();
    expect(result).toEqual({});
  });
