...
describe("cloud history through the socket driver, real SQLite and desktop cache", () => {
  ...
  it("persists failed Git saves independently of turn success through restart and sparse snapshots", async () => {
    const failure = {
      committed: true,
      pushed: false,
      sha: "local-sha",
    ...
    onFrame({ type: "turn.started", sessionId: PROVIDER, turnId: "save-failed", timestamp: T });
    onFrame({
      type: "turn.ended",
      sessionId: PROVIDER,
      turnId: "save-failed",
    ...
    db.pragma("foreign_keys = ON");
    await connect();
    onFrame(snapshot([], { turns: [ended("later-no-receipt")] }));
    onFrame(
      snapshot([], {
        turns: [
          ended("older-save", {
            endedAt: T + 50,
            gitSync: { committed: false, pushed: true, sha: "old-remote-sha" },
          }),
        ],
      })
    );
    ...
    onFrame(
      snapshot([], {
        turns: [
          ended("confirmed-save", {
            endedAt: T + 200,
            gitSync: { committed: false, pushed: false, sha: "confirmed-remote-sha" },
          }),
        ],
      })
    );
    expect(sessionRow()).toMatchObject({ cloud_git_sync_at: T + 200, cloud_git_error: null });
  });

  it("restores a missed Git failure atomically with its cloud transcript", () => {
    const frame = snapshot([message("answer", 0)], {
      turns: [
        ended("turn-1", { gitSync: { committed: false, pushed: false, error: "Backup denied" } }),
      ],
    });
    db.exec(
      `CREATE TRIGGER reject_restored_part BEFORE INSERT ON parts BEGIN SELECT RAISE(ABORT, 'disk failure'); END;`
    );
    onFrame(frame);
    expect(sessionRow()).toMatchObject({ cloud_git_sync_at: null, cloud_git_error: null });
    expect(rows()).toHaveLength(0);
    db.exec("DROP TRIGGER reject_restored_part");
    onFrame(frame);
    expect(sessionRow()).toMatchObject({ status: "idle", cloud_git_error: "Backup denied" });
