      ...
      >
        {sidebarExpanded ? (
          ...
          <CollapsibleTrigger asChild>
            <SidebarMenuButton
              className={cn(
                // Simple, performant: only animate transforms/opacity (GPU-accelerated, no layout reflow)
                ...
                // Hover states: lift for active, opacity for idle (separate transitions for performance)
                isActive && "hover:translate-y-[-2px] transition-transform duration-200",
                isIdle && "hover:opacity-60 transition-opacity duration-200"
              )}
              tooltip={(() => {
