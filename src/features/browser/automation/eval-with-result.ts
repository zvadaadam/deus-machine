...
// Evaluates JavaScript in a Tauri webview and captures the return value
// via WKWebView's native evaluateJavaScript:completionHandler: API.
//
// Architecture:
...
const ASYNC_SENTINEL = "__CONDUCTOR_ASYNC__";
  ...
  const wrappedJs = `(function(){
    try {
      ...
      if (__result && typeof __result === 'object' && typeof __result.then === 'function') {
        window.__conductor_pending = window.__conductor_pending || {};
        window.__conductor_pending['${requestId}'] = {done: false};
        __result.then(function(v) {
          window.__conductor_pending['${requestId}'] = {done: true, value: typeof v === 'string' ? v : String(v)};
        }, function(e) {
          window.__conductor_pending['${requestId}'] = {done: true, error: e.message || String(e)};
        });
...
async function pollAsyncResult(
  ...
  const pollJs = `(function(){
    var p = window.__conductor_pending && window.__conductor_pending['${requestId}'];
    if (!p) return JSON.stringify({done: false});
    if (p.done) {
      delete window.__conductor_pending['${requestId}'];
      return p.error
  ...
  invoke("eval_browser_webview", {
    label,
    js: `delete (window.__conductor_pending || {})['${requestId}']`,
  }).catch(() => {});
