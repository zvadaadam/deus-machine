...
# Validate Deus environment variable
if [ -z "$DEUS_ROOT_PATH" ]; then
    echo -e "${RED}ERROR: DEUS_ROOT_PATH environment variable not set${NC}"
    echo ""
    echo "This script must be run by Deus, which sets this variable automatically."
    echo ""
    exit 1
fi
...
# Copy .env file
if [ -f "$DEUS_ROOT_PATH/.env" ]; then
    cp "$DEUS_ROOT_PATH/.env" .env
    echo -e "${GREEN}✓ Copied .env from root repository${NC}"
elif [ -f "$DEUS_ROOT_PATH/.env.example" ]; then
    cp "$DEUS_ROOT_PATH/.env.example" .env
...
# Copy .mcp.json file
if [ -f "$DEUS_ROOT_PATH/.mcp.json" ]; then
    cp "$DEUS_ROOT_PATH/.mcp.json" .mcp.json
    echo -e "${GREEN}✓ Copied .mcp.json from root repository${NC}"
elif [ -f "$DEUS_ROOT_PATH/.mcp.json.example" ]; then
    cp "$DEUS_ROOT_PATH/.mcp.json.example" .mcp.json
...
echo ""
echo "Or run 'bun run dev' for Electron desktop app"
echo ""
