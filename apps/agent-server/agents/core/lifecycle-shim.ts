...
// Phase A of the @agent-server/core embedding: translate the engine's
// LifecycleEvent stream into deus's PartEvents so the existing
// EventBroadcaster wire format stays byte-compatible while the engine
// underneath is swapped. Deleted when consumers move to LifecycleEvent.

import type { LifecycleEvent, Part as EnginePart } from "@agent-server/protocol";
import type { Part, ReasoningPart, TextPart, ToolPart } from "@shared/messages";
        ...
        return [
          {
            ...
            turnId: event.turnId,
            ...(event.finishReason
              ? { finishReason: event.finishReason as PartEvent extends never ? never : never }
              : {}),
