...
// JavaScript injection for AI visual effects inside WKWebView.
//
...
//
// Ported from mcp-dev-browser's visual-effects-injection.ts, adapted for
// native WKWebView title-channel architecture (no postMessage available).
//
// Usage: eval VISUAL_EFFECTS_SETUP once per page load, then use
// buildMoveCursorJs/buildRippleJs etc. to construct per-action JS strings.
...
export const VISUAL_EFFECTS_SETUP = `(function(){
  ...
  function rippleAt(x, y) {
    try {
      ...
      // Custom iOS-style curve for energetic expansion (Emil: custom bezier > built-in)
      ...
      requestAnimationFrame(function() {
        ...
        setTimeout(function() { try { ring1.remove(); } catch(e) {} }, 300);
      });
      ...
      setTimeout(function() {
        ...
        requestAnimationFrame(function() {
          ...
          setTimeout(function() { try { ring2.remove(); } catch(e) {} }, 400);
        });
      }, 80);
    } catch(e) {}
  }
  ...
  function screenshotFlash(rect) {
    try {
      ...
      setTimeout(function() {
        ...
        setTimeout(function() { try { flash.remove(); } catch(e) {} }, 140);
        ...
        setTimeout(function() {
          ...
          setTimeout(function() { try { tint.remove(); } catch(e) {} }, 650);
        }, 350);
      }, 50);
  ...
  function highlightElement(el) {
    ...
    try {
      ...
      setTimeout(function() {
        box.style.opacity = '0';
        setTimeout(function() { try { box.remove(); } catch(e) {} }, 450);
      }, 300);
    } catch(e) {}
  }
  ...
  // Each band has an asymmetric trailing gradient (long tail above bright line)
  // and an optional clip-path tint that grows behind it like a highlighter.
  // Emil: stagger children, asymmetric timing, converging momentum.
  ...
  function scanPage() {
    try {
      ...
      // Scanner-line design: compact bands with bright leading edge + box-shadow glow.
      // Each band = short gradient tail + bright bottom edge (like a flatbed scanner).
      // Two passes converge at the bottom for dramatic "analysis complete" feel.
      //
      // CRITICAL: transitions must NOT be in the initial cssText. The browser batches
      // inline style writes and skips the "from" state if transition is set before the
      // first paint. Pattern: append with initial state → reflow → add transition + end.
      ...
      // Appends el, forces reflow to establish "from", then sets transition + "to".
      var startTransition = function(el, transitions, endProps) {
        ...
        for (var k in endProps) { el.style[k] = endProps[k]; }
      };
      ...
      // Append tint (no transition needed yet — clip-path transition starts with band)
      ...
      setTimeout(function() {
        try {
          ...
          setTimeout(function() { try { band2.remove(); } catch(e) {} }, P2_MS + 100);
          setTimeout(function() {
  ...
  //
  // The glow physically expands and contracts like it's alive:
  // - box-shadow grows (blur 80→130, spread 15→35) + slight scale(1→1.04)
  // - Creates genuine visual motion, not just brightness flickering
  // - Emil: ease-in-out for on-screen movement, 3.5s natural breathing pace
  // - Emil: interruptible — keeps breathing during fade-out, no snaps
  // - opacity transition handles fade in/out independently of animation
  // ========================================================================
...
/**
 * Build JS to move cursor to an element by data-cursor-ref,
